...
- 클래스는 작아야 한다.
  ...
  - 변경할 이유를 파악하다보면 코드를 추상화 하기 쉬워진다.
  - 도구 상자를 어떻게 관리하고 싶은가?  
  작은 서랍을 많이 두고 기능과 이름이 명확한 컴포넌트를 나눠 넣고 싶은가?  
  아니면 큰 서랍 몇개를 두고 모두를 던져 넣고 싶은가?
  - 큰 클래스 몇 개가 아니라 작은 클래스 여럿으로 이루어진 시스템이 바람직하다
- 응집도
...
- OCP
  - 클래스는 확장에 개방적이로 수정에 폐쇄적이어야한다.
- DIP
  - 클래스 설계원칙
  - 클래스가 상세한 구현이 아닌 추상화에 의존해야한다.

### 소감 💬
- 클래스를 제작할 때 어떻게 제작하는 것이 바람직한지에 대해 알 수 있었다.  
특히 도구를 정리할 때의 비유 부분이 인상깊었다. 평소 기능을 작게 분리할 때 작게 분리하면 왔다갔다하면서 봐야하니 분리하지 말라는 말도 들었는데, 그에 대한 의견을 추가할 수 있을 것 같다.